...
def hybrid_search(
    ...
    rows = list(conn.execute(
        f"SELECT rowid, id, content, project, tags, source_path, created_at "
    ...
    chunk_map = {}
    for row in rows:
        rowid, chunk_id, content, proj, tags, src_path, created_at = row
        age = now - created_at
        decay = 0.5 ** (age / half_life_seconds)
        scores[rowid] *= decay
        chunk_map[rowid] = (chunk_id, content, proj, tags, src_path, created_at)
    ...
    if project:
        scored_rowids = [
            ...
            if rid in chunk_map and chunk_map[rid][2] == project  # [2] = project
        ]
    else:
        scored_rowids = [
    ...
    for rid, score in scored_rowids[:limit]:
        chunk_id, content, proj, tags, src_path, created_at = chunk_map[rid]
        results.append(
            SearchResult(
                id=chunk_id,
                content=content,
